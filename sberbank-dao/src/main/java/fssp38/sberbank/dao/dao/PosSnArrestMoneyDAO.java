 ...
 * Time: 10:17 AM
 */
public class PosSnArrestMoneyDAO {
    DataSource dataSource;
    JdbcTemplate jdbcTemplate;

    public PosSnArrestMoneyDAO(DataSource dataSource) {
        this.dataSource = dataSource;
        jdbcTemplate = new JdbcTemplate(dataSource);
    }

    public List<PosSnArrestMoney> getAll(Long lastIdSnAr) {
        String sql3 =
...
//                " did.id_dbtr_entid in (2,71,95,96,97) and d.id > " + lastIdSnAr + "order by d.id"
...
//                        "o.contr_name\n" +
                        ...
                        "join o_ip_res res on res.id = d.id\n" +
                        "join o_ip_act_arrest_money arr on arr.id = d.id\n" +
                        "join O_IP_ACT_ENDARREST gacc  on gacc.id = d.id   /* O_IP_ACT_ARREST_ACCMONEY - об обращении\n" +
                        "o_ip_act_gaccount_money - о наложении ареста  O_IP_ACT_ENDARREST - о снятии ареста */\n" +
                        "join doc_ip_doc did on did.id = o_ip.ip_id\n" +
                        "join doc_ip di on di.id = o_ip.ip_id\n" +
                        "join entity on di.id_dbtr = entity.entt_id\n" +
                        " where\n" +
                        ...
                        " ))\n" +
                        "\n" +
                        " and d.id >" + lastIdSnAr + " order by d.id    /* 25111000003896  25111000028137   25111002017360  */ ";
...
//            public PosArrestMoney mapRow(ResultSet rs, int rowNum) throws SQLException {
        List<PosSnArrestMoney> posSnArrestMoneyList = jdbcTemplate.query(sql3, new RowMapper<PosSnArrestMoney>(){
            @Override
        public PosSnArrestMoney mapRow(ResultSet rs, int rowNum) throws SQLException {

                PosSnArrestMoney pos = new PosSnArrestMoney();
